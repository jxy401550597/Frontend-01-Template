...
function layout(element) {
	...
	if(mainSpace < 0) {
		...
		flexLines.forEach(items => {
			...
			for(let i = 0; i < items.length; i++) {
				...
				if(itemStyle.flex !== null && itemStyle !== undefined) {
	...
	flexLines.forEach(items => {
		let lineCrossSize = style["align-content"] === "stretch" ? items.crossSpace + crossSpace / flexLines.length : items.crossSpace;
		for(let i = 0; i < items.length; i++) {
			let item = items[i];
			let itemStyle = getStyle(item);
			...
			if(align === "flex-start") {
				...
				itemStyle[crossStart] = crossBase;
				itemStyle[crossEnd] = crossBase + crossSign * (itemStyle[crossStart] !== null && itemStyle[crossSize] !== undefined ? itemStyle[crossSize] : lineCrossSize);
